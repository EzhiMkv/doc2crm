...
| 📄 → 💼 Фото счёта → сделка → каталог | 🔎 Вопрос по документам с цитатами |
|---|---|
| ![Фото счёта → сделка → каталог](docs/screenshots/demo_catalog_find.png) | ![Вопрос с цитатами](docs/screenshots/demo_qa_citations.png) |

| 🛒 Товар приложен к сделке в CRM | 🧮 Агрегация по документам |
|---|---|
| ![Товары в сделке Битрикс24](docs/screenshots/demo_deal_products.png) | ![Агрегация](docs/screenshots/demo_qa_aggregate.png) |

Полный цикл на скриншотах: фото счёта из Telegram → распознанные реквизиты →
сделка #310 в Битрикс24 → `/find` подобрал товары под бюджет «до 30к» →
товар приложен к сделке (вид в CRM справа-ниже).
